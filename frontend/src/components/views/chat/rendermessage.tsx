...
const getStepIcon = (
  ...
  if (is_step_repeated)
    return <AlertTriangle size={16} className="text-blue-800" />;
  if (status === "completed")
    return <CheckCircle size={16} className="text-blue-800" />;
  if (status === "current" && runStatus === "active")
    return <RefreshCw size={16} className="text-blue-800 animate-spin" />;
  if (status === "upcoming")
  ...
  ({
    ...
    return (
      <div className="flex flex-col">
        ...
        <div
          className={`relative border-2 border-transparent hover:border-gray-300 rounded-lg p-2 cursor-pointer overflow-hidden bg-blue-400`}
          onClick={handleToggle}
        >
          <div className="flex items-center w-full">
            <button
              className="flex-none flex items-center justify-center w-8 h-8 rounded-full bg-blue-400 transition-colors"
              onClick={(e) => {
                e.stopPropagation();
                handleToggle();
              }}
      ...
      >
        ...
        >
          <div
            className={`${
              isUser || isUserProxy
                ? `text-primary rounded-2xl bg-blue-400 rounded-tr-sm px-4 py-2 ${
